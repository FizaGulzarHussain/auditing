...
def _story(audit: dict) -> list:
    ...
    story.append(CoverPage(audit))
    story.append(PageBreak())

    # ── Page 2: Core Web Vitals + Projection ─────────────────────────────────
    cwv_panel   = CoreWebVitalsPanel(proj, width=CW)
    proj_panel  = ProjectionPanel(proj, width=CW)

    cwv_section = [
        Spacer(1, 2 * mm),
        Paragraph(_t("CORE WEB VITALS ASSESSMENT",
                      "CORE WEB VITALS BEWERTUNG"), st["H2"]),
        Paragraph(_t(
            "Google uses these three metrics as a direct ranking signal. "
            "Failing any threshold hurts your visibility in search.",
            "Google nutzt diese drei Metriken als direkten Rankingfaktor. "
            "Das Verfehlen eines Schwellenwerts schadet Ihrer Sichtbarkeit."),
            st["BodyG"]),
        Spacer(1, 2 * mm),
        cwv_panel,
        Spacer(1, 3 * mm),
    ...
    proj_section = [
        Paragraph(_t("PERFORMANCE PROJECTION",
                      "LEISTUNGSPROGNOSE"), st["H2"]),
        Paragraph(_t(
            "Estimated improvements based on your site's actual measured metrics, "
            "projected against Fast Edge Cache benchmarks. No code changes required.",
            "Geschaetzte Verbesserungen basierend auf Ihren gemessenen Metriken, "
            "verglichen mit Fast Edge Cache-Benchmarks. Keine Code-Aenderungen."),
            st["BodyG"]),
        Spacer(1, 2 * mm),
        proj_panel,
    ]

    # If both fit on one page, keep together; otherwise let them flow naturally.
    combined_h = _estimate_height(cwv_section + proj_section)
    if combined_h <= _CONTENT_H - 10:
        story.extend(cwv_section)
        story.extend(proj_section)
    else:
        story.extend(cwv_section)
        # Only force a page break before projection if CWV section alone is
        # large enough that projection would have tiny space left.
        cwv_h = _estimate_height(cwv_section)
        if _CONTENT_H - cwv_h < _estimate_height(proj_section) * 0.4:
            story.append(PageBreak())
        story.extend(proj_section)

    story.append(PageBreak())

    # ── Pages 3+: Category detail sections ───────────────────────────────────
    DETAIL_CATS = [
        ("Performance",     "performance",   "#EC4899"),
        ("Speed",           "speed",         "#8B5CF6"),
        ("SEO",             "seo",           "#3B82F6"),
    ...
    RELEVANT_KEYS = {k for _, k, _ in DETAIL_CATS}

    # Track whether we placed anything yet on this "content block"
    first_section = True

    for i, (cat_label, cat_key, cat_hex) in enumerate(DETAIL_CATS):
        cat_data  = bd.get(cat_key) or {}
        score     = cat_data.get("score", 0)
        issues    = _clean(cat_data.get("issues", []))
        strengths = list(cat_data.get("strengths", []))

        if cat_key == "mobile" and not issues:
            issues = [_t(
                "Mobile audit returned an average result — no critical mobile-specific "
                "issues detected. Further manual testing on real devices is recommended.",
                "Mobile-Audit ergab ein durchschnittliches Ergebnis. Weitere manuelle "
                "Tests auf echten Geraeten werden empfohlen.")]

        if not issues and not strengths:
            continue

        # ── Build the section's flowable list ──────────────────────────────
        section_flows = []

        banner = SectionBanner(_cat_label(cat_label), score, cat_hex, width=CW)
        section_flows.append(banner)
        section_flows.append(Spacer(1, 1 * mm))

        if issues:
            section_flows.append(
                Paragraph(_t("Issues Identified", "Identifizierte Probleme"), st["H3"]))
            section_flows.append(Spacer(1, 0.5 * mm))
            for j, iss in enumerate(issues[:15]):
                sev_label_str, sev_col = _sev(iss)
                impact, solution = _rec(iss, bd, proj)
                section_flows.append(
                    IssueCard(_translate_issue(iss), impact, solution,
                              _sev_label(sev_label_str), sev_col))
                if j < len(issues[:15]) - 1:
                    section_flows.append(Spacer(1, 1))

        if strengths:
            strength_block = [Paragraph(_t("Strengths", "Staerken"), st["H3"])]
            for s in strengths[:10]:
                strength_block.append(
                    Paragraph(
                        f'<font color="#059669"><b>&#10003;</b></font> {_safe(_translate_strength(s))}',
                        st["Body"]))
            section_flows.extend(strength_block)

        # Separator (not for the last real section)
        sep_flows = [
            Spacer(1, 2 * mm),
            HRFlowable(width=CW, thickness=0.5, color=RULE),
        ]

        # ── Smart placement ────────────────────────────────────────────────
        # RULE: NEVER wrap the full section in KeepTogether.
        # Doing so causes ReportLab to push the whole section to the next page
        # whenever it doesn't fit in the *remaining* space — leaving large blank
        # gaps.  Instead we only lock the anchor together so the banner never
        # strands alone at the bottom of a page.
        #
        # section_flows layout:
        #   [0] SectionBanner
        #   [1] Spacer(1mm)
        #   [2] "Issues Identified" Paragraph   (if issues exist)
        #   [3] Spacer(0.5mm)                   (if issues exist)
        #   [4] first IssueCard                 (if issues exist)  ← MUST be in anchor
        #   [5] Spacer(1)                        (between cards)
        #   ...
        # If no issues, [2] onwards are strength items.
        #
        # Anchor must contain at minimum: banner + spacer + heading + spacer + first card
        # so that the banner is never left dangling alone or with just a heading below it.
        anchor_end = min(5, len(section_flows))   # indices 0-4 inclusive
        anchor     = section_flows[:anchor_end]
        rest       = section_flows[anchor_end:]

        if first_section:
            # We just had a PageBreak — place anchor directly, then rest freely.
            story.append(KeepTogether(anchor))
            for fl in rest:
                story.append(fl)
            first_section = False
        else:
            # For subsequent sections: guard against a naked banner at the very
            # bottom of a page, then place anchor + rest freely.
            # _SpaceGuard only breaks if remaining space < _MIN_HEADING_GUARD,
            # so most of the time content continues on the same page.
            story.append(_SpaceGuard(_MIN_HEADING_GUARD))
            story.append(KeepTogether(anchor))
            for fl in rest:
                story.append(fl)

        story.extend(sep_flows)

    # ── Business Impact & ROI ─────────────────────────────────────────────────
    # NO hardcoded PageBreak here.  Instead we measure the ROI section height
    # and decide dynamically:
    #   • If it fits in the remaining space on the current page → flow straight in.
    #   • If it doesn't fit but the remaining blank space is large (> 35% of a
    #     full page) → still flow in and let ReportLab split naturally so we
    #     don't waste that space with a forced break.
    #   • If only a tiny sliver remains (< _MIN_HEADING_GUARD) → page break,
    #     because we never want just a heading dangling at the bottom.
    biz_panel   = BusinessImpactPanel(proj, audit)
    biz_heading = [
        Spacer(1, 2 * mm),
        Paragraph(_t("BUSINESS IMPACT & ROI ANALYSIS",
                      "GESCHAEFTSAUSWIRKUNG & ROI-ANALYSE"), st["H2"]),
        Paragraph(_t(
            "Estimated revenue impact of your current site speed, and projected "
            "return on fast.site infrastructure.",
            "Geschaetzter Umsatzeffekt Ihrer aktuellen Ladezeit und projizierter "
            "Return on Investment mit fast.site."), st["BodyG"]),
        Spacer(1, 2 * mm),
    ]
    biz_heading_h = _estimate_height(biz_heading)
    biz_total_h   = biz_heading_h + biz_panel.height

    # _SpaceGuard with biz_total_h: if the remaining space is less than the full
    # ROI block we still flow in (ReportLab will split the panel across pages if
    # needed).  But if remaining space is less than heading + a minimum stub of
    # the panel, force a page break so we don't get an orphaned heading.
    _biz_min_space = biz_heading_h + 60   # heading + at least ~60 pt of panel
    story.append(_SpaceGuard(_biz_min_space))

    story.extend(biz_heading)
    story.append(biz_panel)
    story.append(Spacer(1, 3 * mm))

    # ── Priority Action Plan ───────────────────────────────────────────────────
    all_issues_sorted = []
    sev_order_map = {"CRITICAL": 0, "HIGH": 1, "MEDIUM": 2, "LOW": 3}
    sev_hex_map   = {"CRITICAL": "#DC2626", "HIGH": "#D97706",
                     "MEDIUM": "#0284C7",  "LOW": "#059669"}

    for _lbl, _key, _hex in CATS:
        if _key not in RELEVANT_KEYS:
            continue
        for iss in _clean((bd.get(_key) or {}).get("issues", [])):
            sev_lbl, _ = _sev(iss)
            all_issues_sorted.append((sev_order_map[sev_lbl], sev_lbl, _lbl, iss))
    all_issues_sorted.sort(key=lambda x: x[0])

    # Guard: if remaining space is tiny, start action plan on new page
    story.append(_SpaceGuard(_MIN_HEADING_GUARD + 40))

    pap_heading = [
        Spacer(1, 2 * mm),
        Paragraph(_t("PRIORITY ACTION PLAN", "PRIORITAETSAKTIONSPLAN"), st["H2"]),
        Paragraph(_t(
            f"All <b>{len(all_issues_sorted)} issues</b> ranked by severity. "
            "Resolve CRITICAL items first — they cause the most ranking and revenue damage.",
            f"Alle <b>{len(all_issues_sorted)} Probleme</b> nach Schweregrad sortiert. "
            "Beheben Sie KRITISCHE Punkte zuerst."), st["BodyG"]),
        Spacer(1, 2 * mm),
    ]
    story.extend(pap_heading)

    cat_col_w = 72 if _LANG == "de" else 58
    col_w = [26, 56, cat_col_w, CW - 26 - 56 - cat_col_w - 12]
    tbl_rows = [[
        Paragraph("<b>#</b>", st["Small"]),
        Paragraph(f"<b>{_t('SEVERITY', 'SCHWEREGRAD')}</b>", st["Small"]),
        Paragraph(f"<b>{_t('CATEGORY', 'KATEGORIE')}</b>", st["Small"]),
        Paragraph(f"<b>{_t('ISSUE', 'PROBLEM')}</b>", st["Small"]),
    ]]

    for idx, (_, sev_lbl, cat_lbl, iss_txt) in enumerate(all_issues_sorted):
        short = (iss_txt[:110] + "…") if len(iss_txt) > 110 else iss_txt
        hx = sev_hex_map[sev_lbl]
        tbl_rows.append([
            Paragraph(str(idx + 1), st["Small"]),
            Paragraph(f'<font color="{hx}"><b>{_sev_label(sev_lbl)}</b></font>',
                      st["Small"]),
            Paragraph(_cat_label(cat_lbl), st["Small"]),
            Paragraph(_safe(_translate_issue(short)), st["Small"]),
        ])

    tbl_style = [
        ("BACKGROUND", (0, 0), (-1, 0), DARK),
        ("TEXTCOLOR",  (0, 0), (-1, 0), BLUE_DIM),
        ("FONTSIZE",   (0, 0), (-1, -1), 7.5),
        ("TOPPADDING",    (0, 0), (-1, -1), 3),
        ("BOTTOMPADDING", (0, 0), (-1, -1), 3),
        ("LINEBELOW",  (0, 0), (-1, -1), 0.4, RULE),
        ("VALIGN",     (0, 0), (-1, -1), "MIDDLE"),
        ("REPEATROWS", (0, 0), (0, 0), 1),
    ]
    for ri in range(1, len(tbl_rows)):
        bg = LIGHT if ri % 2 == 1 else WHITE
        tbl_style.append(("BACKGROUND", (0, ri), (-1, ri), bg))

    action_tbl = Table(tbl_rows, colWidths=col_w, repeatRows=1)
